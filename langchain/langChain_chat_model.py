...
# 대화 히스토리를 구현하기 위해 4가지 타입인, SystemMessage,HumanMessage, AIMessage가 주로 사용됨
# SystemMessage : 챗봇에게 개발자가 명령을 내리기 위해서 사용. 챗봇의 역할이나 대화에 대한 가이드 혹은 제약 사항 설정
# HumanMessage: 챗봇에게 사용 자가 질의하는 메시지
# AIMessage : 챗봇이 답변한 대화 내용.
...
### 다음 대화를 이어서 호출 하고 싶을 때. , 기존 질문과 AI 응답까지 합해서 다음 메시지를 부를때 넣어 주는식(하지만 이러면 토큰수가 커져서 요금 문제 있음)
print("-"*30)
...
### 위와 같이 messages에 직접 대화들을 추가 하는 방식도 있지만 LangChain에서 ChatMessageHistory 라는 클래스를 제공하는데 이를 사용 할 수 도있음.
...
# 주의: API 호출 시 토큰 수 제한이 있음. 
# 대화가 길어질 경우, 토큰 수 제한을 넘지 않도록 관리 필요.
# 필요한 경우, 이전 대화 내용을 요약하는 방법으로 토큰 수를 관리할 수 있음.
